...
fun Route.paymentSplitRoutes() {
    val service = PaymentSplitService(PaymentSplitRepository())

    route("/payment-splits") {
        get {
            call.requireAuth()
        ...
        post {
            ...
            val request = call.receive<PaymentSplitCreateRequest>()
        ...
        put("{id}") {
            ...
            val id = call.parameters["id"]?.let { UUID.fromString(it) }
                ?: throw BadRequestException("invalid id")
            val patch = call.receive<PaymentSplitUpdateRequest>()
